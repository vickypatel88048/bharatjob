...
const normalizeType = (type = "") => type.toString().trim().toLowerCase().replace(/[_\s]+/g, "-");
    ...
    <>
      ...
      <div className="min-h-screen bg-[#eeeeee] font-[Arial,sans-serif]">
        <div className="w-full max-w-[1000px] mx-auto bg-white min-h-screen">
          ...
          <div className="text-center px-5 py-3"><p className="text-[11px] sm:text-[12px] leading-5 text-slate-700">BharatJobs Official Website - Get Online Form, Results, Admit Card, Answer Key, Syllabus, Career News, Sarkari Yojana, Scholarship, Sarkari Notice and Latest Government Jobs.</p></div>
          <div className="flex justify-center"><span className="inline-flex items-center gap-1 text-[11px] font-bold"><span className="w-2 h-2 rounded-full bg-red-600 animate-pulse" />LIVE</span></div>
          <div className="text-center mt-2"><a href="#" className="inline-block bg-[#20c76a] text-white text-[11px] font-bold px-5 py-[6px] rounded-full no-underline">Join WhatsApp Channel</a></div>
          <div className="text-center mt-2"><a href="#" className="text-blue-700 underline text-[13px] font-bold">BharatJobs Tools</a></div>
          <div className="px-3 sm:px-5 mt-4"><form onSubmit={(e) => e.preventDefault()} className="flex max-w-[650px] mx-auto"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search Government Jobs, Results, Admit Cards..." className="flex-1 h-[40px] border border-slate-300 px-3 text-[12px] outline-none focus:border-[#000080]" /><button type="submit" className="w-[85px] h-[40px] bg-[#000080] text-white text-[12px] font-bold">Search</button></form></div>
          {search.trim() ? <section className="px-3 sm:px-5 mt-4"><SectionTitle>Search Results</SectionTitle><div className="border border-slate-300 border-t-0">{searchResults.length ? searchResults.map((post) => <PostItem key={post._id} post={post} />) : <div className="text-center py-7 text-xs text-slate-500">No matching posts found.</div>}</div></section> : <>
            <section className="px-3 sm:px-5 mt-3"><div className="grid grid-cols-2 sm:grid-cols-4 gap-[2px]">{featuredPosts.map((post, index) => { const colors = ["bg-[#e53935]", "bg-[#fb8c00]", "bg-[#00897b]", "bg-[#1e88e5]", "bg-[#8e24aa]", "bg-[#d81b60]", "bg-[#43a047]", "bg-[#3949ab]"]; return <Link key={post._id} to={`/post/${post.slug}`} className={`${colors[index % colors.length]} min-h-[42px] flex items-center justify-center text-center text-white px-2 py-2 text-[10px] sm:text-[11px] font-bold leading-4 no-underline hover:opacity-90`}>{post.title}</Link>; })}</div></section>
            <main className="px-3 sm:px-5 mt-5">{loading ? <div className="text-center py-10 text-sm text-slate-500">Loading...</div> : <><div className="grid grid-cols-1 md:grid-cols-3 gap-[5px]"><HomeSection title="Latest Jobs" posts={getPosts("job")} view="/jobs" /><HomeSection title="Results" posts={getPosts("result")} view="/results" /><HomeSection title="Admit Cards" posts={getPosts("admit-card")} view="/admit-card" /></div><div className="grid grid-cols-1 md:grid-cols-2 gap-[5px] mt-[5px]"><HomeSection title="Answer Key" posts={getPosts("answer-key")} view="/answer-key" /><HomeSection title="Admission" posts={getPosts("admission")} view="/admission" /></div>{posts.length > 0 && <section className="mt-7"><SectionTitle>Latest Posts</SectionTitle><div className="border border-slate-300 border-t-0">{posts.slice(0, 15).map((post) => <PostItem key={post._id} post={post} />)}</div></section>}</>}</main>
            <section className="px-4 sm:px-6 mt-6"><p className="text-[12px] leading-6 text-slate-700"><strong>BharatJobs 2026 :</strong> Find latest government job vacancies, Sarkari exam results, online forms, admit cards, answer keys, syllabus, admission notifications and government recruitment updates.</p><p className="text-[12px] leading-6 text-slate-700 mt-2">BharatJobs is an informational platform. Candidates should always verify the details from the official notification and official website of the concerned department.</p></section>
            <section className="px-4 sm:px-6 mt-5"><div className="text-center"><h2 className="text-[17px] font-bold text-slate-800">Top BharatJobs Pages</h2></div><div className="text-center text-[12px] leading-7 mt-2"><Link to="/jobs" className="text-blue-700 hover:underline">Latest Jobs</Link>{" | "}<Link to="/results" className="text-blue-700 hover:underline">Government Results</Link>{" | "}<Link to="/admit-card" className="text-blue-700 hover:underline">Admit Card</Link>{" | "}<Link to="/answer-key" className="text-blue-700 hover:underline">Answer Key</Link>{" | "}<Link to="/admission" className="text-blue-700 hover:underline">Admission</Link></div></section>
          </>}
