...
const Home = () => {
  ...
  const searchResults = useMemo(() => { const query = search.trim().toLowerCase(); if (!query) return []; return posts.filter((post) => post.title?.toLowerCase().includes(query)).slice(0, 15); }, [search, posts]);
  const count = (type) => posts.filter((post) => post.type === type).length;
  const isSearching = search.trim().length > 0;

  return (<>
    <Helmet><title>BharatJobs - Latest Government Jobs, Results & Admit Card</title><meta name="description" content="Latest government jobs, results, admit cards, answer keys, admissions and recruitment notifications on BharatJobs." /></Helmet>
    <div className="min-h-screen bg-[#eeeeee] font-[Arial,sans-serif]"><div className="w-full max-w-[1000px] mx-auto bg-white min-h-screen">
      ...
      <main className="bj-home">
        ...
        {!isSearching ? <>
          <div className="bj-quick-links"><QuickLink to="/jobs" title="Latest Jobs" value={count("job")} tone="red" /><QuickLink to="/results" title="Results" value={count("result")} tone="blue" /><QuickLink to="/admit-card" title="Admit Card" value={count("admit-card")} tone="green" /><QuickLink to="/answer-key" title="Answer Key" value={count("answer-key")} tone="orange" /><QuickLink to="/admission" title="Admission" value={count("admission")} tone="purple" /></div>
          ...
          <section className="bj-important"><div className="bj-section-title"><div><small>BHARATJOBS UPDATES</small><h2>Important Updates</h2></div><Link to="/jobs">View All</Link></div><div className="bj-update-grid">{featured.length ? featured.map((post, i) => <Link className="bj-update-item" key={post._id} to={`/post/${post.slug}`}><span>{String(i + 1).padStart(2, "0")}</span><b>{post.title}</b>{isNew(post) && <em>NEW</em>}</Link>) : <Empty text="No featured updates available" />}</div></section>
          {loading ? <div className="bj-loading">Loading latest updates...</div> : <><div className="bj-main-heading"><span>GOVERNMENT RECRUITMENT</span><h2>Latest Government Jobs</h2><Link to="/jobs">View All Jobs →</Link></div><section className="bj-portal-grid"><PortalSection title="Latest Jobs" link="/jobs" posts={byType("job", 10)} /><PortalSection title="Latest Results" link="/results" posts={byType("result", 8)} /><PortalSection title="Admit Card" link="/admit-card" posts={byType("admit-card", 8)} /></section><section className="bj-portal-grid bj-lower-grid"><PortalSection title="Answer Key" link="/answer-key" posts={byType("answer-key", 8)} /><PortalSection title="Admission" link="/admission" posts={byType("admission", 8)} /><PortalSection title="Syllabus" link="/syllabus" posts={byType("syllabus", 8)} /></section></>}
