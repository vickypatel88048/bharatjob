          ...
          <main className="bj-home">
            ...
            {!isSearching ? <>
              ...
              {loading ? <div className="bj-loading">Loading latest updates...</div> : <>
                ...
                <section className="bj-job-board"><div className="bj-job-main"><div className="bj-board-head"><b>Recruitment Notification</b><span>Recently Published</span></div>{byType("job", 12).map((post) => <PostRow key={post._id || post.slug} post={post} />)}{byType("job").length === 0 && <Empty />}</div><aside className="bj-side-stack"><MiniSection title="Latest Results" link="/results" posts={byType("result", 6)} /><MiniSection title="Admit Card" link="/admit-card" posts={byType("admit-card", 6)} /></aside></section>
...
const MiniSection = ({ title, link, posts }) => <section className="bj-mini"><div className="bj-mini-head"><b>{title}</b><Link to={link}>View all</Link></div>{posts.length > 0 ? posts.map((post) => <PostRow key={post._id || post.slug} post={post} />) : <Empty />}</section>;
